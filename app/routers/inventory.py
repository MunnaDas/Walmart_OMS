...
def movements(product_id: int, db: Session = Depends(get_db)):
    ids = [row.id for row in db.query(Inventory).filter_by(product_id=product_id).all()]
    if not ids:
        return []
    return db.query(InventoryMovement).filter(InventoryMovement.inventory_id.in_(ids)).order_by(InventoryMovement.created_at.desc()).all()
