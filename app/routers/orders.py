...
from app.models import Allocation, Fulfillment, Inventory, InventoryMovement, Order, OrderItem, Product, User
...
def create_order(data: OrderIn, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if not data.items:
        raise HTTPException(400, "Order must contain items")
    if user.role != "ADMIN" and data.customer_id != user.id:
        raise HTTPException(403, "Customers can only create orders for themselves")
    ...
    if idempotency_key:
        ...
        if existing:
            ...
            assert_order_access(existing, user)
            return existing

    try:
        order = Order(customer_id=data.customer_id, idempotency_key=idempotency_key, idempotency_request_hash=payload_hash if idempotency_key else None)
        db.add(order)
        db.flush()
        total = Decimal("0.00")
        for item in data.items:
            ...
            query = db.query(Inventory).filter(Inventory.product_id == product.id, Inventory.available_quantity > 0).order_by(Inventory.available_quantity.desc(), Inventory.id)
            ...
            for stock in query.all():
                ...
                db.add(Allocation(order_id=order.id, order_item_id=order_item.id, warehouse_id=stock.warehouse_id, quantity=take))
                db.add(InventoryMovement(inventory_id=stock.id, movement_type="RESERVATION", quantity=take, reference_type="ORDER", reference_id=str(order.id)))
                remaining -= take
            if remaining:
...
def list_orders(limit: int = 50, offset: int = 0, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    total = query.count()
    items = query.order_by(Order.id.desc()).offset(offset).limit(limit).all()
...
def cancel_order(order_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        for allocation in db.query(Allocation).filter_by(order_id=order_id, status="ALLOCATED").all():
            ...
            db.add(InventoryMovement(inventory_id=stock.id, movement_type="RELEASE", quantity=allocation.quantity, reference_type="ORDER", reference_id=str(order.id)))
